...
_frow:=2
...
#ifdef A_FA
  if dok_p_r="F"
    @ 5,0,11,79 BOX '«ƒ∂∫∫ ∫∫ ' CODEPAGE 'PLMAZ' 
    _frow:=7
 #ifdef A_FK
   if dok_fk
 ...
 #undef D_KPR
    ...
    @ _frow+1,0,_frow+4,79 BOX if(pozycja>D_LP1.and.!nowydm,'ÃÕπ∫Ωƒ”∫ ','ÃÕπ∫ºÕ»∫ ') CODEPAGE 'PLMAZ' 
...
if _flp>D_LPVAL(dm->pozycja) .or. if(_flp=0, dm->pozycja >D_LP0,_fi#D_LPVAL(pozycja)) .and. D_LPVAL(pozycja)>0
   ...
   @ _frow+2,_fco1,_frow+4,_fco2 BOX if(pozycja>D_LP1,'∫ ∫∫Ωƒ”∫ ','∫ ∫∫ºÕ»∫ ') CODEPAGE 'PLMAZ' color _SBKGR
   _fpopkey:=.f.
   dok11(_f)
   _fkey:=K_ESC
   inkey()
