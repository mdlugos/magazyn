...
#undef D_KPR
    ...
    @ _frow+1,0,_frow+4,79 BOX if(pozycja>D_LP1.and.!nowydm,'ÃÕπ∫ºƒ»∫ ','ÃÕπ∫ºÕ»∫ ')
...
/*
      ...
      if dbseek(KEY_PAR+str(nk,5),.f.) .and. i<>recno()
         j:=if(nk<0,-1,1)
         alarm("Znalazàem inny dokument o tym samym numerze !;Zmieniam numer dokumentu na pierwszy wolny.",,,3)
      ...
      if nowydm
         ...
         if data<=max(DatY->d_z_mies1,DatY->data_gran)
            if month(max(DatY->d_z_mies1,DatY->data_gran)+1 D_OLZA)=1
              alarm("Wàaònie zamkni©to rok!",,,3)
              break
            endif
            alarm("Wàaònie zamkni©to miesi•c!;Zmieniam dat© dokumentu.",,,3)
            _fkey:=K_PGUP
...
#else
          // break lub .t.
          aeval(rarr,{|i|dbgoto(i),if(reclock(.F.,"NIE POTRAFI® ZMIENIè NUMERU W POZYCJI"+D_LPSTR(POZYCJA)),nr_dowodu:=str(nk,5),)})
          unlock
#endif
          select DM
      endif
...
begin sequence
       ...
       if type("exp_od")="D" .and. type("exp_do")="D" .and. exp_od=CTOD(n_f) .and. exp_do=dd
          ...
          darr:=array(lastrec())
          exec {||message(10),darr[recno()]:=pad(index,len(main->index))+str(ilosc,10,ILDEC)+" "+proc_vat+"% "+str(cena,10,A_ZAOKR)+" zà "+str(licznik,3)+" X "+nazwa+str(wartosc,10,2)}
          use
       endif
       end sequence
#else
...
*/
          ...
          px:=pv
            ...
            aadd(ames,message("NIE ZMIENIùEM CENY ZBYTU, PONIEWAΩ TOWAR W KASIE I ZMIANA STAWKI VAT."))
...
if _flp>D_LPVAL(dm->pozycja) .or. if(_flp=0, dm->pozycja >D_LP0,_fi#D_LPVAL(pozycja)) .and. D_LPVAL(pozycja)>0
   ...
   @ _frow+2,_fco1,_frow+4,_fco2 BOX if(pozycja>D_LP1,'∫ ∫∫ºƒ»∫ ','∫ ∫∫ºÕ»∫ ') color _SBKGR
   _fpopkey:=.f.
   dok11(_f)
   _fkey:=K_ESC
   inkey()
