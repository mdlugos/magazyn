...
_frow:=2
 ...
 #undef D_KPR
    ...
    @ _frow+1,0,_frow+4,79 BOX hb_UTF8ToStrBox(if(pozycja>D_LP1.and.!nowydm,'╠═╣║╜─╙║ ','╠═╣║╝═╚║ '))
...
//          endif
   ...
   @ row(),32 get dazapl
    ...
    @ _frow-1,50 GET xml_ksef PICTURE "@KS28" SEND block:={|x|if(x=NIL.or.!empty(x).or.ksef_getfa(n_ksef,,@x)=NIL.and.alarm('Błąd:'+HB_EOL()+hb_translate(x,'UTF8',))<>NIL,xml_ksef,xml_ksef:=x)}
#endif
...
#else
           dbeval(x,{||RECLOCK(.F.,"NIE POTRAFIĘ ZMIENIO NUMERU KONTA POZYCJI"+D_LPSTR(POZYCJA) ,.F.)},{||KEY_DOK+nr_dowodu=dm->(KEY_DOK+nr_dowodu)})
           unlock
#endif
           else
...
#undef D_LAN0
   ...
   if _fskip*(_fl-_fj+2)+_frow>maxrow()
      ...
      ++_fl
      @ (_fl-_fj)*_fskip+_frow,_fco1,(_fl-_fj+1)*_fskip+_frow,_fco2 BOX hb_UTF8ToStrBox('║ ║║╝═╚║ ') color _sbkgr
...
if _flp>D_LPVAL(dm->pozycja) .or. if(_flp=0, dm->pozycja >D_LP0,_fi#D_LPVAL(pozycja)) .and. D_LPVAL(pozycja)>0
   ...
   @ _frow+2,_fco1,_frow+4,_fco2 BOX hb_UTF8ToStrBox(if(pozycja>D_LP1,'║ ║║╜─╙║ ','║ ║║╝═╚║ ')) color _SBKGR
   _fpopkey:=.f.
   dok11(_f)
   _fkey:=K_ESC
   inkey()
