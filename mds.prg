 ...
 *      23        //CTRL END
 ...
 //      27        //ESC
  ...
  if _sprompt=NIL
    ...
    if _snagl=NIL
      _snagkol:=0//_scol1
      _snagl:=padl(field(1),dbstruct()[1,3],'─')+"┬"+field(2)
    endif
  endif
...
*  PO PIERWSZYM WYWOLANIU _SINFO, O ILE NIE ZOSTYALY ZMIENIONE
  ...
  DO WHILE .T.
    ...
    IF DTOS(A->data)>=A_DEMO .AND. RECNO()%75=0
      alarm("Wersja demonstracyjna.;W sprawie zakupu pełnej wersji;skontaktuj się z autorem programu.",,3,3)
//      _skey:=27
    ENDIF
#endif
...
FUNCTION _SDOL(_s,_skey)
    ...
    ELSE
      ...
      ENDDO
      if _sef
        @ _srow2,_scol1-1 SAY '╘'+REPLICATE('═',_scoln)+'╛' COLOR _SRAMKA
...
IF _si<_srown
  ...
  ENDIF
  @ _srow2-1,_scol1-1,_srow2,_scol2 BOX hb_UTF8ToStrBox('│ ││┘─└│') COLOR _SRAMKA
...
IF _si<_srown
  IF _srow1=_srowb+1
   ...
   ELSE
    --_srow1
  ENDIF
  @ _srow1-1,_scol1-1,_srow1,_scol2 BOX hb_UTF8ToStrBox('┌─┐││ ││') COLOR _SRAMKA
  @ _srow1-1,_scol1+_snagkol SAY _snagl COLOR _SRAMKA
  ++_si
 ELSE
  IF _sef
...
if x=NIL
  ...
  _srow2:=_srow1+x
  _si:=x
  @ _srow1-1,_scol1-1,_srow2,_scol2 BOX hb_UTF8ToStrBox('┌─┐│┘─└│') COLOR _SRAMKA
  @ _srow1-1,_scol1+_snagkol SAY _snagl COLOR _SRAMKA
  @ _srow2,_scol1+_snagkol SAY R1TO4(_snagl) COLOR _SRAMKA
  _sef=.F.
  _sbf=.F.
