#include "inkey.ch"
#include "dm_form.ch"
#define R1TO4(x) _stopka(x,'Ä','Á')
#define R1TO3(x) _stopka(x,'Í','Ï')
#define R1TO2(x) STRTRAN(STRTRAN(R1TO1(x),'Â','Ñ'),'Ä','Í')
...
FUNCTION _SDOL(_s,_skey)
    ...
    ELSE
      ...
      ENDDO
      if _sef
        @ _srow2,_scol1-1 SAY 'Ô'+REPLICATE('Í',_scoln)+'¾' CODEPAGE 'PLMAZ' COLOR _SRAMKA
...
FUNCTION _sgora(_s,_skey)
    ...
    IF _sm>1
     ...
     ELSE
      ...
      ENDDO
      if _sbf
        @ _srow1-1,_scol1-1 SAY 'Õ'+REPLICATE('Í',_scoln)+'¸' CODEPAGE 'PLMAZ' COLOR _SRAMKA
  ...
  return .f.
    ...
    IF _si=0
      ...
      elseif _srowe>_srow2
        ++_srow2
        @ _srow2-1,_scol1-1,_srow2,_scol2 BOX '³ ³³¾ÍÔ³' CODEPAGE 'PLMAZ' COLOR _SRAMKA
        @ _srow2,_scol1+_snagkol SAY R1TO3(_snagl) CODEPAGE 'PLMAZ' COLOR _SRAMKA
        oef:=.t.
      else
        --_srow1
        @ _srow1-1,_scol1-1,_srow1,_scol2 BOX 'ÕÍ¸³³ ³³' CODEPAGE 'PLMAZ' COLOR _SRAMKA
        @ _srow1-1,_scol1+_snagkol SAY R1TO2(_snagl) CODEPAGE 'PLMAZ' COLOR _SRAMKA
        obf:=.t.
      endif
      _si:=1
