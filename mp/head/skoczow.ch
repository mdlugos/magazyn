...
#define STANY   INDX_MAT
   ...
   //#define A_JMALT
   #define A_KODY "CPV"
   #define A_KODVAL {|g,y,z,_s|z:=select(),sel('CPV','CPV_CODE'),_s:=array(_sLEN),_sret:=!g:changed .or. dbseek(y),_spocz:=UPPER(trim(y)),_sbeg:=if(asc(y)>64,12,1),if(_sbeg=1,,ordsetfocus((indexord()%2)+1)),_slth:=len(_spocz),_sret:=_sret.or.szukam(_s).and.(kw:=_FIELD->code,.t.),dbselectar(z),.t.}
