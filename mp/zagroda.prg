...
DO WHILE file(ko)
 ...
 IF lower(right(ko,4))='.rtf'
    ...
    field->dost_odb:=trim(firmy->nazwa) + ' * ' + firmy->longname
    field->kontrahent:=fi
...
IF og = 1
    ...
    a:=pad(if(empty(b['billing_address','company']),b['billing_address','firstname']+' '+b['billing_address','lastname'], b['billing_address','company']),len(firmy->longname))
    if d == 'F'
       if !empty(c) .and. firmy->(ordsetfocus('firm_nip'),dbseek(c,.f.)) .and. trim(firmy->ident)==c
       elseif !empty(c) .and. firmy->(ordsetfocus('firm_lng'),dbseek(a,.f.))
	 LOCK IN FIRMY
	 firmy->ident:=c
	 ...
	 c := firmy->(recno())
         UNLOCK IN FIRMY
	 firmy->(ordsetfocus('firm_naz'))
	 d:=len(e:=firmy->nazwa)
	 do while !empty(e) .and. d > 16 .and. firmy->(dbseek(UpP(e),.f.)) .and. firmy->(recno())!=c
	    firmy->(dbgoto(c))
            LOCK IN FIRMY
            e:= firmy->nazwa := pad(subs(a,d := 1 + HB_At(a,' ',d)),len(firmy->nazwa))
            UNLOCK IN FIRMY
	 enddo
       endif
 ...
 */
 ...
 IF c<>0
   ...
   field->ilosc_f:=-1
   field->nr_zlec:=subs(dm->nr_faktury,3,6)
