...
@  8,maxcol()/2+16 PROMPT '1. Inwentaryzacja' MESSAGE 'Drukowanie "Arkusza spisu z natury" dla okre˜lonych gaˆ©zi kodu materiaˆu.'
@  9,maxcol()/2+16 PROMPT '2. Dzieä pocz.   ' MESSAGE 'Wyb¢r dnia, od kt¢rego wy˜wietlane s¥ ruchy.'
@ 10,maxcol()/2+16 PROMPT '3. Zamkni©cie.   ' MESSAGE 'Zamkni©cie miesi¥ca (lub roku), kontrola stan¢w na podst. ruch¢w.'
@ 11,maxcol()/2+16 PROMPT '4. Otwarcie.     ' MESSAGE 'Otwarcie pomyˆkowo zamkni©tego miesi¥ca lub dokumentu.'
@ 12,maxcol()/2+16 PROMPT '5. Symbole i kody' MESSAGE 'Przegl¥danie i porawianie baz danych, opis¢w, symboli, kod¢w, haseˆ, itp.'
IF STARY_ROK#NIL
@ 13,maxcol()/2+16 PROMPT '6. Zmiana roku.  ' MESSAGE 'Wej˜cie do poprzedniego roku lub powr¢t do bie¾¥cego roku.'
 ELSE
@ 13,maxcol()/2+16 PROMPT '6. Stary rok.    ' message 'Wej˜cie do zeszˆego roku, zmiany przenoszone na bie¾¥cy rok po zamkni©ciu.'
ENDIF
...
//#endif
   ...
   if stary_rok#NIL
      txt:={"POWRàT"} //nie wiem, jak g©boko zakopany
   else
      txt:={}
   endif
...
#ifdef PLWIN
    #include   'hbgtinfo.ch'
      aczojs({"98x32","80x25 (Norm)","Zmieniaj ilo˜† linii","Zmieniaj wielko˜† czcionki","Peˆny ekran (Alt+Enter)"},"",@m)
      do case
      case m<2
        SetMode(32,98)
      case m=2
...
#undef D_Z1
  ...
  if dz1>DatY->d_z_mies1
    select 0
    nuse dm
    if !filock(.t.,"Prosz© wszystkich u¾ytkownik¢w sieci o wyj˜cie na chwil© z wprowadzania/poprawiania dokument¢w, aby mo¾na byˆo upewni† si©, ¾e nikt nie zmienia danych z zamykanego okresu. Po rozpocz©ciu zamykania mo¾na b©dzie powr¢ci† do przerwanej pracy.;Omiä - tylko kontrola stan¢w")
       dz1:=DatY->d_z_mies1
    else
       DatY->data_gran:=dz1
       DatY->(dbgoto(1))
...
#define D_JMA A_JMALTTOT(ILOSC,MAIN->nr_zlec,4,x)
  ...
  begin sequence
      ...
      ? "Rà½NICA W STANIE KOãCOWYM"
      ? "BYO",STAN,INDX_MAT->JM,WARTOSC,"zˆ"
    sTAN:=s
#ifndef STANY
#ifdef A_ZAGRODA
    ...
    ?? " JEST",STAN,INDX_MAT->JM,WARTOSC,"zˆ"
    ?
    ?
  else
...
#endif
   set color to (_sel)
   ? "SPRAWDZENIE ZGODNO—CI STANU KOãCOWEGO I BILANSU OTWARCIA NOWEGO ROKU."
   ?
   set color to (_snorm)
   canfix:=D_Z1=DatY->d_z_mies1 .and. 1=alarm("Czy przenie˜† STAN KOãCOWY;na BILANS OTWARCIA NOWEGO ROKU",{"TAK","NIE"})
#undef D_Z1
      select 0
  ...
  DO WHILE !EOF() .OR. !NOWY->(EOF())
    ...
    IF !NOWY->(EOF()).and.NR_MAG+INDEX>NOWY->(NR_MAG+INDEX) .OR. EOF()
      ...
      IF NOWY->ZAMKN_ROKU#0
        ...
        ? "STAN KOãCOWY:  BRAK !"
#ifdef A_WA
        ? "STAN BO:     ",NOWY->ZAMKN_ROKU,NOWY_IDX->jm,NOWY->WART_ROKU,"zˆ"
        if canfix
           lock in NOWY
           ? "DOKONANO KOREKTY BO !"
...
#ifdef A_WA
        ? "STAN KOãCOWY:",ZAMK_MIES1,indx_mat->jm,WART_MIES1,"zˆ"
#else
        ? "STAN KOãCOWY:",ZAMK_MIES1,indx_mat->jm
#endif
        ? "STAN BO:       BRAK !"
        if canfix
           NOWY->(dbappend())
         ...
         if NOWY_IDX->data_popr<=indx_mat->data_popr;
          ...
          .and.(.not."DATA"$fieldname(key) .or. NOWY_IDX->(FIELDget(KEY))<FIELDGET(KEY))
            ...
            if empty(s)
               ...
               ? "Rà½NICA ZAWARTO—CI PàL: "
               if canfix
                  LOCK IN NOWY_IDX
               endif
            endif
            ? fieldname(key),"STAN KOãCOWY",fieldget(key),"STAN BO",NOWY_IDX->(fieldget(key))
...
#endif
#ifdef A_DDBF
   nuse daty READONLY EXCLUSIVE //¾eby nikt si© nie pchaˆ do "roboczy"
   if !used()
      break
   endif
#endif
...
#endif
#ifdef A_DDBF
   nuse daty READONLY EXCLUSIVE alias "XXXX" //¾eby nikt si© nie pchaˆ do "roboczy"
   if !used()
      break
   endif
#endif
...
@ 9,10 say "Prosz© poda† numer magazynu i gaˆ¥«:" 
...
if !tak("Czy drukowa† pozycje o stanie zerowym w/g kartoteki magazynu")
  ...
  if !dbseek(gal,.f.)
    alarm("Brak takich materiaˆ¢w",,,3)
    return
  endif
...
@ 11,10 say "Ta gaˆ¥« liczy "+LTRIM(STR(ILE))+" pozycji, "+LTRIM(STR(INT(ILE/30+.99)))+" arkuszy."
seek gal
ark=1
...
?
#ifdef A_SHORTIND
? ccpi(7)+"ÄÄÄÂÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_12LPI)
?  "   ³    ³                       ³    ³",spec(P_SUPON)+"ILO—     ³STAN      ³WARTO—   ³ DATA I NUMER OST.   ³Rà½NICA MI¨DZY STANEM³",spec(P_SUPOFF)
...
?  "   ³    ³                       ³    ³",spec(P_SUPON)+"W DNIU    ",spec(P_SUPOFF)+"³",spec(P_SUPON)+"KARTOTEKI ",spec(P_SUPOFF)+"³",spec(P_SUPON)+"KARTOTEKI ",spec(P_SUPOFF)+"ÃÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ´"
?  "         NAZWA                        ",spec(P_SUPON)+"SPISU      MAGAZYNU   MAGAZYNU    PRZYCHàD   ROZCHàD   NIEDOBORY   NADWY½KI ",spec(P_SUPOFF)+" SPISUJ¤CEGO"
?  "ÄÄÄÁÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_8LPI)
...
#else
#ifdef A_KTM
? ccpi(7)+"ÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_12LPI)
?  "   ³                ³                       ³    ³",spec(P_SUPON)+"ILO—     ³STAN/wart. ³ DATA I NUMER OST.  ³Rà½NICA MI¨DZY STANEM³",spec(P_SUPOFF)
...
?  "   ³                ³                       ³    ³",spec(P_SUPON)+"W DNIU    ",spec(P_SUPOFF)+"³",spec(P_SUPON)+"KARTOTEKI ",spec(P_SUPOFF)+"ÃÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ´"
?  "                     NAZWA                        ",spec(P_SUPON)+"SPISU      MAGAZYNU    PRZYCHàD   ROZCHàD   NIEDOBORY   NADWY½KI",spec(P_SUPOFF)+"  SPISUJ¤CEGO"
?  "ÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_8LPI)
do while lp<=min(ark*30,ile)
...
#else
#ifdef A_OLZA
? ccpi(7)+"ÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_12LPI)
?  "   ³              ³                       ³    ³",spec(P_SUPON)+"ILO—     ³STAN/wart. ³ DATA I NUMER OST.  ³Rà½NICA MI¨DZY STANEM³",spec(P_SUPOFF)
?  "LP. KOD MATERIAU  OKRE—LENIE PRZEDMIOTU   JEDN ",spec(P_SUPON)+"STWIERDZ.  WEDUG       DOK. MAGAZYNOWEGO   FAKT. I KSI¨GOWYM   ",spec(P_SUPOFF)+"  UWAGI ZESPOU"
?  "   ³              ³                       ³    ³",spec(P_SUPON)+"W DNIU    ",spec(P_SUPOFF)+"³",spec(P_SUPON)+"KARTOTEKI ",spec(P_SUPOFF)+"ÃÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ´"
?  "    INDEKS SWW     NAZWA                        ",spec(P_SUPON)+"SPISU      MAGAZYNU    PRZYCHàD   ROZCHàD   NIEDOBORY   NADWY½KI",spec(P_SUPOFF)+"  SPISUJ¤CEGO"
?  "ÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_8LPI)
do while lp<=min(ark*30,ile)
? str(lp,3)+"|"+tran(INDEX,"@R "+ INDEXPIC )+"|"+LEFT(INDX_MAT->NAZWA,23)+"|"+INDX_MAT->jM+"|          |"+if(sflag,D_ILOUT(stan),space(10))+"|"+ostp(@DOP)+"|"+ostr(@DOR)+"|          |          |"
? "   |              |"+pad(substr(INDX_MAT->NAZWA,24),23)+"|    |          |"+if(sflag,D_WAOUT(stan),space(10))+"|"+dop+"|"+dor+"|          |          |"
#else
...
? ccpi(7)+;
   "----------------------------------------------------------------------------------------------------------------------------------------",spec(P_12LPI)
?  "                                  |    |",spec(P_SUPON)+"ILO—     ",spec(P_SUPOFF)+"|",spec(P_SUPON)+"STAN/wart.",spec(P_SUPOFF)+"|"+P_SUPON+"  DATA I NUMER OST.  "+spec(P_SUPOFF)+"|"+spec(P_SUPON)+"Rà½NICA MI¨DZY STANEM"+spec(P_SUPOFF)+"|"
...
?  "      SWW  NAZWA                        ",spec(P_SUPON)+"SPISU      MAGAZYNU    PRZYCHàD   ROZCHàD   NIEDOBORY   NADWY½KI",spec(P_SUPOFF)+"  SPISUJ¤CEGO"
...
#else
? ccpi(7)+"ÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_12LPI)
...
?  "   ³            ³                       ³    ³",spec(P_SUPON)+"W DNIU    ",spec(P_SUPOFF)+"³",spec(P_SUPON)+"KARTOTEKI ",spec(P_SUPOFF)+"ÃÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ´"
?  "    INDEKS SWW   NAZWA                        ",spec(P_SUPON)+"SPISU      MAGAZYNU    PRZYCHàD   ROZCHàD   NIEDOBORY   NADWY½KI",spec(P_SUPOFF)+"  SPISUJ¤CEGO"
?  "ÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ",spec(P_8LPI)
#ifndef A_INOWA
?? spec(P_SUPON)
#endif
do while lp<=min(ark*30,ile)
...
? "Podpis osoby odpowiedzialnej materialnie                                Podpisy zespoˆu spisuj¥cego"
