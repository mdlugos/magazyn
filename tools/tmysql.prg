...
#define IdenT(x) iif(x="`",x,"`"+x+"`")
#define UnIdenT(x) iif(x="`",SubStr(x,2,Len(x)-2),x)
...
METHOD FieldType( nNum ) CLASS TMySQLRow
   ...
   IF nNum >= 1 .AND. nNum <= Len( ::aFieldStruct )
      ...
      CASE MYSQL_TYPE_TINY
         RETURN iif(::aFieldStruct[ nNum ][ MYSQL_FS_LENGTH ] == 1,"L","N")
...
METHOD MakePrimaryKeyWhere() CLASS TMySQLRow
   ...
   FOR nI := 1 TO Len( ::aFieldStruct )
      ...
      IF hb_bitAnd( ::aFieldStruct[ nI ][ MYSQL_FS_FLAGS ], PRI_KEY_FLAG ) == PRI_KEY_FLAG .OR. ;
         ...
         cWhere += IdenT(::aFieldStruct[ nI ][ MYSQL_FS_NAME ]) + "="
   ...
   IF ::nResultHandle != NIL
      ...
      IF ::aRow != NIL
         ...
         FOR i := 1 TO ::nNumFields
            ...
            CASE MYSQL_TYPE_TINY
               IF ::aRow[ i ] == NIL
                  ::aRow[ i ] := iif( ::aFieldStruct[ i ][ MYSQL_FS_TYPE ] == 1, ".F.", "0" )
               ENDIF
               ::aRow[ i ] := Val( ::aRow[ i ] ) <> 0
               EXIT
            ...
            CASE MYSQL_TYPE_DATE
               ...
               ELSE
                  ...
                  ::aRow[ i ] := hb_CToD( ::aRow[ i ], "YYYYY-MM-DD")
               ENDIF
               EXIT
...
METHOD FieldPos( cFieldName ) CLASS TMySQLQuery
   ...
   cUpperName := Upper( UnIdenT(cFieldName) )
...
METHOD New( nSocket, cQuery, cTableName ) CLASS TMySQLTable
   ...
   ::cTable := Lower( UnIdenT( cTableName ) )
   ::aOldValue := {}
...
METHOD Update( oRow, lOldRecord, lRefresh ) CLASS TMySQLTable
   ...
   LOCAL cUpdateQuery := "UPDATE " + IdenT(::cTable) + " SET "
   LOCAL i
   ...
   IF oRow == NIL // default Current row
      ...
      FOR i := 1 TO  ::nNumFields
         IF !( ::aOldValue[ i ] == ::FieldGet( i ) )
            cUpdateQuery += IdenT(::aFieldStruct[ i ][ MYSQL_FS_NAME ]) + "=" + ClipValue2SQL( ::FieldGet( i ) ) + ","
         ENDIF
      NEXT
      ...
      IF lOldRecord
         ...
         FOR nI := 1 TO Len( ::aFieldStruct )
            cWhere += IdenT(::aFieldStruct[ nI ][ MYSQL_FS_NAME ]) + "=" + ClipValue2SQL( ::aOldValue[ nI ] ) + " AND "
         NEXT
         // remove last " AND "
   ...
   ELSE
      IF oRow:cTable == ::cTable
         ...
         FOR i := 1 TO Len( oRow:aRow )
            IF oRow:aDirty[ i ]
               cUpdateQuery += IdenT(oRow:aFieldStruct[ i ][ MYSQL_FS_NAME ]) + "=" + ClipValue2SQL( oRow:aRow[ i ] ) + ","
            ENDIF
         NEXT
         ...
         IF lOldRecord
            ...
            FOR nI := 1 TO Len( oRow:aFieldStruct )
               cWhere += IdenT(oRow:aFieldStruct[ nI ][ MYSQL_FS_NAME ]) + "=" + ClipValue2SQL( oRow:aOriValue[ nI ] ) + " AND "
            NEXT
            // remove last " AND "
            cWhere := Left( cWhere, Len( cWhere ) - 5 )
            cUpdateQuery += cWhere
...
METHOD Delete( oRow, lOldRecord, lRefresh ) CLASS TMySQLTable
   ...
   LOCAL cDeleteQuery := "DELETE FROM " + IdenT(::cTable), i
   ...
   ELSE
      ...
      IF oRow:cTable == ::cTable
         ...
         IF lOldRecord
            ...
            FOR nI := 1 TO Len( oRow:aFieldStruct )
               cWhere += IdenT(oRow:aFieldStruct[ nI ][ MYSQL_FS_NAME ]) + "="
               // use original value
...
METHOD Append( oRow, lRefresh ) CLASS TMySQLTable
   ...
   LOCAL cInsertQuery := "INSERT INTO " + IdenT(::cTable) + " ("
   LOCAL i
   ...
   IF oRow == NIL // default Current row
      ...
      FOR i := 1 TO ::nNumFields
         IF ::aFieldStruct[ i ][ MYSQL_FS_FLAGS ] != AUTO_INCREMENT_FLAG
            cInsertQuery += IdenT(::aFieldStruct[ i ][ MYSQL_FS_NAME ]) + ","
         ENDIF
      NEXT
      ...
      IF oRow:cTable == ::cTable
         ...
         FOR i := 1 TO Len( oRow:aRow )
            IF oRow:aFieldStruct[ i ][ MYSQL_FS_FLAGS ] != AUTO_INCREMENT_FLAG
               cInsertQuery += IdenT(oRow:aFieldStruct[ i ][ MYSQL_FS_NAME ]) + ","
            ENDIF
         NEXT
         // remove last comma from list
         cInsertQuery := Left( cInsertQuery, Len( cInsertQuery ) - 1 ) + ") VALUES ("
...
METHOD GetBlankRow( lSetValues ) CLASS TMySQLTable
   ...
   FOR i := 1 TO ::nNumFields
      ...
      CASE MYSQL_TYPE_TINY
         aRow[ i ] := iif(::aFieldStruct[ i ][ MYSQL_FS_LENGTH ] == 1,.F.,0)
         EXIT
...
METHOD MakePrimaryKeyWhere() CLASS TMySQLTable
   ...
   FOR nI := 1 TO Len( ::aFieldStruct )
      ...
      IF hb_bitAnd( ::aFieldStruct[ nI ][ MYSQL_FS_FLAGS ], PRI_KEY_FLAG ) == PRI_KEY_FLAG .OR. ;
         ...
         cWhere += IdenT(::aFieldStruct[ nI ][ MYSQL_FS_NAME ]) + "=" + ClipValue2SQL( ::aOldValue[ nI ] )
      ENDIF
   NEXT
...
CREATE CLASS TMySQLServer
   ...
   METHOD New( cServer, cUser, cPassword, nPort, cDatabase, nFlags, cCharset )  // Opens connection to a server, returns a server object
...
METHOD New( cServer, cUser, cPassword, nPort, cDatabase, nFlags ) CLASS TMySQLServer
   ...
   ::cPassword := cPassword
   ::cDBName := cDatabase
   ::nSocket := mysql_real_connect( cServer, cUser, cPassword, cDatabase, nPort, nFlags )
   ::lError := .F.
...
METHOD CreateTable( cTable, aStruct, cPrimaryKey, cUniqueKey, cAuto ) CLASS TMySQLServer
   ...
   ::cCreateQuery := "CREATE TABLE " + Lower( IdenT(cTable) ) + " ("

   FOR i := 1 TO Len( aStruct )

      ::cCreateQuery += IdenT(aStruct[ i ][ DBS_NAME ])

      SWITCH aStruct[ i ][ DBS_TYPE ]
      CASE "C"
         IF aStruct[ i ][ DBS_LEN ] < 40
            ::cCreateQuery += " char(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
         ELSE
            ::cCreateQuery += " varchar(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
         ENDIF
            ::cCreateQuery += Eval( cNN, aStruct[ i ] ) + iif( aStruct[ i ][ DBS_NAME ] == cPrimaryKey, " NOT NULL ", "" ) + ","
         EXIT
      ...
      CASE "L"
         ::cCreateQuery += " tinyint(1)" + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "N"
#if 0
         IF aStruct[ i ][ DBS_DEC ] == 0
            ::cCreateQuery += " int(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")" + Eval( cNN, aStruct[ i ] ) + iif( aStruct[ i ][ DBS_NAME ] == cPrimaryKey, " NOT NULL ", "" ) + iif( aStruct[ i ][ DBS_NAME ] == cAuto, " auto_increment ", "" ) + ","
         ELSE
            ::cCreateQuery += " real(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + "," + hb_ntos( aStruct[ i ][ DBS_DEC ] ) + ")" + Eval( cNN, aStruct[ i ] ) + ","
         ENDIF
#endif
         IF aStruct[ i ][ DBS_DEC ] == 0 .AND. aStruct[ i ][ DBS_LEN ] <= 18
            DO CASE
            CASE aStruct[ i ][ DBS_LEN ] == 1
               ::cCreateQuery += " smallint(1)"
            CASE aStruct[ i ][ DBS_LEN ] == 2
               ::cCreateQuery += " tinyint(2)"
            CASE aStruct[ i ][ DBS_LEN ] <= 4
               ::cCreateQuery += " smallint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            CASE aStruct[ i ][ DBS_LEN ] <= 6
               ::cCreateQuery += " mediumint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            CASE aStruct[ i ][ DBS_LEN ] <= 9
               ::cCreateQuery += " int(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            OTHERWISE
               ::cCreateQuery += " bigint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            ENDCASE
            ::cCreateQuery += Eval( cNN, aStruct[ i ] ) + iif( aStruct[ i ][ DBS_NAME ] == cPrimaryKey, " NOT NULL ", "" ) + iif( aStruct[ i ][ DBS_NAME ] == cAuto, " auto_increment ", "" ) + ","
         ELSE
            ::cCreateQuery += " decimal(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + "," + hb_ntos( aStruct[ i ][ DBS_DEC ] ) + ")" + Eval( cNN, aStruct[ i ] ) + ","
         ENDIF
         EXIT

      CASE "D"
         ::cCreateQuery += " date " + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "B"
         ::cCreateQuery += " double "  + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "I"
         ::cCreateQuery += " mediumint " + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      OTHERWISE
         ::cCreateQuery += " char(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")" + Eval( cNN, aStruct[ i ] ) + ","

      ENDSWITCH

   NEXT
   IF cPrimarykey != NIL
      ::cCreateQuery += " PRIMARY KEY (" + IdenT(cPrimaryKey) + "),"
   ENDIF
   IF cUniquekey != NIL
      ::cCreateQuery += " UNIQUE " + IdenT(cUniquekey) + " (" + IdenT(cUniquekey) + "),"
   ENDIF
...
METHOD CreateIndex( cName, cTable, aFNames, lUnique ) CLASS TMySQLServer
   ...
   cCreateQuery += IdenT(cName) + " ON " + Lower( IdenT(cTable) ) + " ("

   FOR i := 1 TO Len( aFNames )
      cCreateQuery += IdenT(aFNames[ i ]) + ","
   NEXT
...
METHOD DeleteIndex( cName, cTable ) CLASS TMySQLServer
   ...
   LOCAL cDropQuery := "DROP INDEX " + IdenT(cName) + " FROM " + Lower( IdenT(cTable) )
...
METHOD DeleteTable( cTable ) CLASS TMySQLServer
   ...
   LOCAL cDropQuery := "DROP TABLE " + Lower( IdenT(cTable) )
...
METHOD TableStruct( cTable ) CLASS TMySQLServer
   ...
   aStruct := {}
   nRes := mysql_list_fields( ::nSocket, Lower(IdenT(cTable)) )

   IF ! Empty( nRes )
      FOR i := 1 TO mysql_num_fields( nRes )

         aField := mysql_fetch_field( nRes )
         aSField := Array( DBS_ALEN + 1)
         aSField[DBS_NAME] := aField[ i ][ MYSQL_FS_NAME ]
         aSField[DBS_TYPE] := aField[ i ][ MYSQL_FS_TYPE ]
         aSField[DBS_LEN] := aField[ i ][ MYSQL_FS_LENGTH ]
         aSField[DBS_DEC] := aField[ i ][ MYSQL_FS_DECIMALS ]
         aSField[DBS_ALEN + 1] := aField[ i ]

         SWITCH ::aField[ i ][ MYSQL_FS_TYPE ]
            CASE MYSQL_TYPE_TINY
               aSField[DBS_TYPE] :=  iif(aField[ i ][ MYSQL_FS_LENGTH ]==1,"L","N")
               EXIT
...
STATIC FUNCTION ClipValue2SQL( Value )
   ...
   RETURN "NULL"       // NOTE: Here we lose values we cannot convert
