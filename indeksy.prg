...
#ifdef A_ANKER
 #define nazwA (field->nazwa+strpic(field->cenA,11,2,"@E ")+" zà "+field->proc_vat+"%")
#endif
...
#ifdef A_SWW
 #ifdef A_VIEWCZAK
  #define TOPLINE 'Kodƒƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬dn.¬C.Zak.'+LARMG+'¬ƒƒƒƒƒStan¬ƒjmƒ¬Cena'
 #else
  #ifdef A_VIEWVAT
   #define TOPLINE 'Kodƒƒ¬NazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒVat'+LARMG+'¬ƒƒƒƒƒStan¬ƒjmƒ¬Cena'
  #else
   #define TOPLINE 'Kodƒƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒStan¬ƒjmƒ¬Cena'
  #endif
 #endif
 #ifdef A_SHARP
  #undef TOPLINE
  #define TOPLINE 'Kodƒƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬Klaw¬ƒƒƒƒƒStan¬ƒjmƒ¬Cena'
 #endif
#else
 #ifdef A_SHORTIND
      #ifdef A_KODY
       #define TOPLINE 'Kod¬Nazwa'+repl('ƒ',40-len(KoD))+'¬'+pad(A_KODY,len(KoD),'ƒ')+'¬ƒƒƒƒƒƒStan¬ƒjmƒ¬Cena'
       #undef D_KODY
       #define D_KODY 48
      #else
       #define TOPLINE 'Kod¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒƒStan¬ƒjmƒ¬Cena'
      #endif
 #else
  #ifdef A_KTM
   #ifdef A_OBR
    #undef LARMG
     ...
     #define LARMG 'ƒ'
    #endif
    #define TOPLINE 'Kod materiaàuƒƒ¬NazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒNr rys i p¢àkiƒ'+LARMG+'¬ƒƒƒƒƒStan¬jm'
   #else
    #define TOPLINE 'Kod materiaàuƒƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒStan¬jm'
   #endif
  #else
   #ifdef A_OLZA
    #define TOPLINE 'Kod materiaàu¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒƒStan¬jm'
   #else
    #ifdef A_ANKER
     #ifdef A_VIEWVAT
      #define TOPLINE 'Kod kresk.ƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬Kodƒƒ¬VatƒƒƒƒƒƒStan¬ƒjmƒ¬Cena'
     #else
      #ifdef A_VIEWCZAK
       #define TOPLINE 'Kod kresk.ƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬Kodƒƒ¬C.Zak¬ƒƒƒStan¬ƒjmƒ¬Cena'
      #else
#ifdef STANY
       #define TOPLINE 'Kod kresk.ƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬'+pad(A_KODY,len(KoD),'ƒ')+'¬ƒƒƒStan¬ƒjmƒ¬Cena'
#else
       #define TOPLINE 'Kod kresk.ƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬'+pad(A_KODY,len(KoD),'ƒ')+LARMG+'¬ƒƒƒStan¬ƒjmƒ¬Cena'
#endif
      #endif
     #endif
    #else
     #ifdef A_7
      #ifdef A_KODY
       #define TOPLINE 'Kodƒƒƒ¬Nazwa'+repl('ƒ',49-len(KoD))+'¬'+pad(A_KODY,len(KoD),'ƒ')+'¬ƒƒƒƒƒStan¬jm'
       #define D_KODY 55
      #else
       #define TOPLINE 'Kodƒƒƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒStan¬ƒjmƒ¬Cena'
      #endif
     #else
      #define TOPLINE 'Kod mater.ƒ¬Nazwaƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ'+LARMG+'¬ƒƒƒƒƒƒStan¬jm'
     #endif
    #endif
   #endif
  #endif
...
if(len(trim(nazwA))>38,chr(26)," "),left(nazwA,30)+if(len(trim(nazwA))>30,chr(26)," ")+left(nr_rys,7)+" ")+polka) ,45) + if(""=INDX_MAT->UWAGI,RET,"&")+subs(STANY->NR_MAG,2)
...
#ifdef A_TRWALOSC
 #define TRWALTXT IF(WAZNOSC>0.AND.STANY->STAN>0.AND.STANY->DATA_PRZY+WAZNOSC<DA,"",IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET))
#else
 #ifdef A_WA
 ...
 #define D_CF8
  ...
  #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(if(il=NIL,indx_mat->cenA,cenA) D_CF8,9,CEOKR,"@E ",.t.)
 #else
  #ifdef A_WA
   #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(D_WA / D_ST D_CF8,9,CEOKR,"@E ",.t.)
  #else
   #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(if(il=NIL,indx_mat->cenA,cenA) D_CF8,9,CEOKR,"@E ",.t.)
  #endif
 #endif
   ...
   #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(if(il=NIL,indx_mat->cenA,cenA) D_CF8,6,CEOKR,"@E ")
     ...
     #else
      #ifdef A_WA
       #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(D_WA / D_ST D_CF8,8,CEOKR,"@E ",.t.)
      #else
       #define SHORTTXT +IF(STANY->DATA_ZMIAN>DatY->data_gran,"˛",RET)+strpic(if(il=NIL,indx_mat->cenA,cenA) D_CF8,8,CEOKR,"@E ",.t.)
      #endif
     #endif
...
//#endif
  ...
  CASE _skey=-2 .and. _sret#2// F3 - MAG_BIEZ
      ...
      ENDIF
        ...
        _sret:=2
    REFRESH(,_s)
    @ _srow2,_scol1+1 say if(_sfor=NIL,"WSZYSTKIE MATERIAùY ","STAN NIEZEROWY ")+trim(magazyny[mag_poz]) COLOR _slinia
  ...
  CASE _skey=-3 .AND. _sret#1// f4   INDX_MAT
      ...
      _sfor:=NIL
      REFRESH(,_s)
      @ _srow2,_scol1+1 say "MATERIAùY Z WSZYSTKICH MAGAZYN‡W" COLOR _slinia
      _sret=1
  ...
  CASE _skey=-1 .or. _skey=7 // POKAZ/UKRYJ ZEROWKI
   ...
   CASE _skey=13
    ...
    _spform={|p,l|TranR(right(p,l), INDEXPIC )}
            ...
            refresh(0,_s)
            getlist:={}
            bl:=message("NIE POTRAFI® PRZEPISAè DO WùASNEJ KARTOTEKI;- INDEKS ZAJ®TY;PROSZ® PODAè NOWY SYTMBOL:")
         ...
         if hlink[3]
            getlist:={}
            _skey:=message("PROSZ® PODAè PROCENT MARΩY:    %")
...
#endif
#ifdef A_OLZA
    szukam({1,max(0,maxcol()-79),maxrow(),maxcol(),0,0,"Data¬ƒDokument¬Zlecenieƒ¬ƒPrzych¢d¬ƒƒRozch¢d¬ƒƒƒ"+WANAZ+"¬"+smiaR+"¬"+str(wb,10,CEOKR),{|D,_s|ltab(D,_s)},{|_skey,_s|tab(_skey,_s)},NR_MAG+INDEX+dtos(DatY->DATA_GRAN)+"˛",NR_MAG+INDEX+"@"})
#else
    szukam({1,max(0,maxcol()-77),maxrow(),maxcol(),0,0,"Data¬ƒƒDokument¬Koszty¬ƒPrzych¢d¬ƒRozch¢dƒ¬ƒƒƒ"+WANAZ+"¬"+smiaR+"¬"+str(wb,10,CEOKR),{|D,_s|ltab(D,_s)},{|_skey,_s|tab(_skey,_s)},NR_MAG+INDEX+dtos(DatY->DATA_GRAN)+"˛",NR_MAG+INDEX+"@"})
#endif
#undef smiaR
    set filter to
    select main
   ...
   case _skey=-5
      ...
      @ bl[1]+1,bl[2]+20 Say "Podaj wartoòÜ:"
      read
      window(bl)
      if readkey()=K_ESC .or. ALARM("CZY NA PEWNO CHCESZ ZAMIENI§ WSZYSTKIE TOWARY ZACZYNAJ‰CE SI® NA:"+_spocz,{"TAK","NIE"},2,2)#1
         return .f.
      endif
      k_pola:=a
  ...
  txt:=message("Licz© stan,;prosz© czekaÜ.")
  seek _spocz
  _skey:=0
