...
local j,x,y
             b:forcestable()
             //j:=b:colpos-1
             j:=hb_HKeyAt(n,b:colpos-1)
             if valtype(hb_HGetDef(w,j))='L' .and. !w[j]
                k:= - K_TAB
                return
             endif
...
win:=window(i,j)
       ...
       for j:=1 to len(n)
         ...
         b:addcolumn(c)
             ...
             if valtype(hb_HGetDef(w,k))='L' .and. w[k]
                w[k]:=NIL
             endif
